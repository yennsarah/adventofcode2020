...
val passports = input.split("\n\n")
    .map { it.replace("\n", " ") }
    .map { parseToPassport(it) }

println("Passports: ${passports.joinToString("\n")}")
...
enum class EyeColor { amb, blu, brn, gry, grn, hzl, oth }
enum class Unit(val rawName: String) { inch("in"), cm("cm"), none("") }
data class Height(val height: Int?, val unit: Unit)

data class DataWithValidation<T>(val data: T?, val isValid: () -> Boolean)
 ...
 */
data class Passport(
    val ecl: DataWithValidation<EyeColor?>,
    val pid: DataWithValidation<String?>,
    val eyr: DataWithValidation<Int?>,
    val hcl: DataWithValidation<String?>,
    val byr: DataWithValidation<Int?>,
    val iyr: DataWithValidation<Int?>,
    val cid: DataWithValidation<String?>,
    val hgt: DataWithValidation<Height?>,
) {
    fun isValid(): Boolean {
        return ecl.isValid() &&
                pid.isValid() &&
...
fun parseToPassport(passportData: String): Passport {
    val eclRaw = findDataEntry(passportData, "ecl")
    val ecl = EyeColor.values().firstOrNull { it.name == eclRaw }
    val pid = findDataEntry(passportData, "pid")
    val eyrRaw = findDataEntry(passportData, "eyr")
    val eyr = eyrRaw.toIntOrNull()
    val hcl = findDataEntry(passportData, "hcl")
    val byr = findDataEntry(passportData, "byr").toIntOrNull()
    val iyr = findDataEntry(passportData, "iyr").toIntOrNull()
    val cid = findDataEntry(passportData, "cid")
    val hgtRaw = findDataEntry(passportData, "hgt")
    val hgt: Height = when {
        hgtRaw.contains(Unit.cm.rawName) -> {
            Height(hgtRaw.replace(Unit.cm.rawName, "").toIntOrNull(), Unit.cm)
        }
        hgtRaw.contains(Unit.inch.rawName) -> {
            Height(hgtRaw.replace(Unit.inch.rawName, "").toIntOrNull(), Unit.inch)
        }
        else -> {
            Height(hgtRaw.toIntOrNull(), Unit.none)
        }
    }

    return Passport(
        ecl = DataWithValidation(ecl, { ecl != null }),
        pid = DataWithValidation(pid, { pid.count() == 9 && pid.matches(Regex("^[0-9]*$")) }),
        eyr = DataWithValidation(eyr, { (2020..2030).contains(eyr) }),
        hcl = DataWithValidation(hcl, { hcl.matches(Regex("^#([a-fA-F0-9]{6})$")) }),
        byr = DataWithValidation(byr, { (1920..2002).contains(byr) }),
        iyr = DataWithValidation(iyr, { (2010..2020).contains(iyr) }),
        cid = DataWithValidation(cid, { true }),
        hgt = DataWithValidation(hgt, {
            when (hgt.unit) {
                Unit.inch -> (59..76).contains(hgt.height)
                Unit.cm -> (150..193).contains(hgt.height)
                Unit.none -> false
            }
        }),
    )
...
fun findDataEntry(data: String, entryTag: String): String {
    val entryTagPosition = data.indexOf("$entryTag:")
    val position = entryTagPosition + "$entryTag:".count()
    if (entryTagPosition != -1) {
        var endOfTag = data.indexOf(" ", position) - 1
        if (endOfTag < 0) endOfTag = data.length - 1

        return data.slice(position..endOfTag)
    }
    return ""
}
