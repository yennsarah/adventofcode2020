...
data class PuzzleInput(val positions: Pair<Int, Int>, val char: Char, val password: CharArray)

val inputLines = input.split("\n")
val puzzleInputs = inputLines.map { line ->
    val split = line.split(" ")

    val rangeStringSplitted = split[0].split("-")
    val positions =
        Pair((rangeStringSplitted[0].toIntOrNull() ?: 1) - 1, (rangeStringSplitted[1].toIntOrNull() ?: 1) - 1)
    PuzzleInput(positions, split[1].first(), split[2].toCharArray())
}

val validPasswords = puzzleInputs.count { validate(it) }
println("validPasswords $validPasswords")

fun validate(puzzleInput: PuzzleInput): Boolean {
    val inFirstPosition = puzzleInput.password[puzzleInput.positions.first] == puzzleInput.char
    val inSecondPosition = puzzleInput.password[puzzleInput.positions.second] == puzzleInput.char
