...
data class Passport(
    val ecl: String, val pid: String, val eyr: String, val hcl: String,
    val byr: String, val iyr: String, val cid: String, val hgt: String,
) {
    fun isValid(): Boolean {
        return ecl.isNotBlank() &&
                pid.isNotBlank() &&
                eyr.isNotBlank() &&
                hcl.isNotBlank() &&
                byr.isNotBlank() &&
                iyr.isNotBlank() &&
                // cid.isNotBlank() && - treat as valid if missing.
                hgt.isNotBlank()
    }
}

fun parseToPassport(data: String): Passport {
    val cleanedData = data.replace("\n", " ")

    return Passport(
        findDataEntry(cleanedData, "ecl"),
        findDataEntry(cleanedData, "pid"),
        findDataEntry(cleanedData, "eyr"),
        findDataEntry(cleanedData, "hcl"),
        findDataEntry(cleanedData, "byr"),
        findDataEntry(cleanedData, "iyr"),
        findDataEntry(cleanedData, "cid"),
        findDataEntry(cleanedData, "hgt"),
    )
}
