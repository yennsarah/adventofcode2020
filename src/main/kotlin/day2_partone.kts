...
data class PuzzleInput(val range: IntRange, val char: Char, val password: CharArray)
...
val puzzleInputs = inputLines.map { line ->
    ...
    val range = IntRange(rangeStringSplitted[0].toIntOrNull() ?: 0, rangeStringSplitted[1].toIntOrNull() ?: 0)
    PuzzleInput(range, split[1].first(), split[2].toCharArray())
}
...
fun validate(puzzleInput: PuzzleInput): Boolean {
    val charCount = puzzleInput.password.count { it == puzzleInput.char }
    return puzzleInput.range.contains(charCount)
}